...
from dataframes.data import DataInMem, _detect_types
from ak_logger import logger

# disable logging
logger.setLevel(logging.WARNING)

class TestDataInMem(unittest.TestCase):
    def setUp(self):
    ...
    def test_init_with_skiplines(self):
        ...
        data1 = DataInMem(filepath1)
        data2 = DataInMem(filepath2, options=options)

        # filters_applied should be excluded
        self.assertFalse("filters_applied" in data1.columns)
        self.assertFalse("filters_applied" in data2.columns)
        self.assertTrue(data1.is_loaded)
        self.assertTrue(data1.is_loaded)
        
        # should use dask
        self.assertTrue(isinstance(data1.data, pd.DataFrame))
        self.assertTrue(isinstance(data2.data, pd.DataFrame))
    ...
    def test_init_with_txt_tab_pipe(self):
        ...
        data1Tab = DataInMem(filepathTab, options=optionsTab)
        data2Tab = DataInMem(filepathTab)
        data1Pipe = DataInMem(filepathPipe, options=optionsPipe)
        data2Pipe = DataInMem(filepathPipe)
        ...
        self.assertTrue(data1Tab.is_loaded)
        self.assertTrue(data2Tab.is_loaded)
        self.assertTrue(data1Pipe.is_loaded)
        self.assertTrue(data2Pipe.is_loaded)

        # should use pandas
        self.assertTrue(isinstance(data1Tab.data, pd.DataFrame))
        self.assertTrue(isinstance(data2Tab.data, pd.DataFrame))
        self.assertTrue(isinstance(data1Pipe.data, pd.DataFrame))
        self.assertTrue(isinstance(data2Pipe.data, pd.DataFrame))
    ...
    def test_init_with_txt_tab_linedelim(self):
        ...
        data = DataInMem(filepath, options=options)
    ...
    def test_init_with_txt_tab_thous(self):
        ...
        dataT = DataInMem(filepathT, options=optionsT)
        dataTC = DataInMem(filepathTC, options=optionsTC)
        dataTC_err = DataInMem(filepathTC, options=optionsT) # load without removing comments
        dataTCE = DataInMem(filepathTCE, options=optionsTCE)
        dataTCE_err = DataInMem(filepathTCE, options=optionsT) # load without removing comments and escape
        ...
        self.assertTrue(isinstance(dataT.data, pd.DataFrame))
        self.assertTrue(isinstance(dataTC.data, pd.DataFrame))
        self.assertTrue(isinstance(dataTC_err.data, pd.DataFrame))
        self.assertTrue(isinstance(dataTCE.data, pd.DataFrame))
        self.assertTrue(isinstance(dataTCE_err.data, pd.DataFrame))
    ...
    def test_init_with_txt_thous_dec(self):
        ...
        data1 = DataInMem(filepath1, options=options1)        
        data2 = DataInMem(filepath2, options=options2)
    ...
    def test_apply_filter(self):
        ...
        self.assertGreaterEqual(data.filter_data['col-1'].min(), 0)
        self.assertLess(data.data['col-1'].min(), 0)
        
        self.assertLessEqual(data.filter_data['col-2'].max(), 0)
        self.assertGreater(data.data['col-2'].max(), 0)
    ...
    def test_apply_nominal_filter(self):
        ...
        data.apply_nominal_filter('col-0', ['a', 'c'], inc=1)
        uni = data.filter_data['col-0'].unique()
        fulluni = data.data['col-0'].unique()
    ...
    def test_apply_merge_single_col(self):
        ...
        left_df = DataInMem(left_fname)
        right_df = DataInMem(right_fname)

        how = 'inner'
        left_on ='a'
        right_on ='a1'
        inner_jdf = DataInMem.apply_merge(left_df, right_df,
                                          left_on=left_on,
                                          right_on=right_on,
                                          how=how);

        how = 'outer'
        outer_jdf = DataInMem.apply_merge(left_df, right_df,
                                          left_on=left_on,
                                          right_on=right_on,
                                          how=how);
        
        how = 'left'
        left_jdf = DataInMem.apply_merge(left_df, right_df,
                                         left_on=left_on,
                                         right_on=right_on,
                                         how=how);
        
        how = 'right'
        right_jdf = DataInMem.apply_merge(left_df, right_df,
                                          left_on=left_on,
                                          right_on=right_on,
                                          how=how);
        
        self.assertEqual(inner_jdf.shape[0], 999)
        self.assertFalse(inner_jdf['b1'].isnull().any())
        self.assertFalse(inner_jdf['b'].isnull().any())
        
        self.assertEqual(outer_jdf.shape[0], 1001)
        self.assertTrue(outer_jdf['b1'].isnull().any())
        self.assertTrue(outer_jdf['b'].isnull().any())

        self.assertEqual(left_jdf.shape[0], 1000)
        self.assertTrue(left_jdf['b1'].isnull().any())
        self.assertFalse(left_jdf['b'].isnull().any())

        self.assertEqual(right_jdf.shape[0], 1000)
        self.assertFalse(right_jdf['b1'].isnull().any())
        self.assertTrue(right_jdf['b'].isnull().any())
    ...
    def test_apply_multi_merge_single_col(self):
        ...
        A_df = DataInMem(A_fname)
        B_df = DataInMem(B_fname)
        C_df = DataInMem(C_fname)
        
        how = 'inner'
        A_on ='a'
        B_on ='a1'
        C_on = 'a2'
        inner_jdf = DataInMem.apply_multi_merge([A_df, B_df, C_df],
                                                on=[[A_on], [B_on], [C_on]],
                                                how=how, suffix=['_x', '_y', '_z']);

        how = 'outer'
        outer_jdf = DataInMem.apply_multi_merge([A_df, B_df, C_df],
                                                on=[[A_on], [B_on], [C_on]],
                                                how=how, suffix=['_x', '_y', '_z']);

        self.assertEqual(inner_jdf.shape[0], 998)
        self.assertFalse(inner_jdf['b1_y'].isnull().any())
        self.assertFalse(inner_jdf['b1_z'].isnull().any())
        self.assertFalse(inner_jdf['b'].isnull().any())
        
        self.assertEqual(outer_jdf.shape[0], 1002)
        self.assertTrue(outer_jdf['b1_y'].isnull().any())
        self.assertTrue(outer_jdf['b1_z'].isnull().any())
        self.assertTrue(outer_jdf['b'].isnull().any())
    ...
    def test_apply_custom(self):
        ...
        min_eps = data.data['eps'].min()
        data.apply_custom('eps', 'eps + 100')
        
        self.assertEqual(data.data['eps'].min(), min_eps + 100)
    ...
    def test_create_derived(self):
        ...
        self.assertEqual(data.data['eps_p_100'].min(), min_eps + 100)
    ...
    def test_get_sampled_data(self):
        ...
        data = DataInMem(filepath, delay_load=True)
        ...
        sdf = data.get_sampled_data(nrows=100)
        self.assertGreaterEqual(sdf['eps2'].min(), 0)
        self.assertEqual(sdf.shape[0], 100)
    ...
    def test_checkpoint(self):
        ...
        self.assertGreaterEqual(data.filter_data['col-2'].min(), 0)
        ...
        self.assertLess(data.filter_data['col-2'].min(), 1)
        self.assertFalse(data._is_transformed['col-2'])
    ...
    def test_reset_data(self):
        ...
        data.reset_data()
        self.assertLess(data.filter_data['col-1'].min(), 0)
    ...
    def test_apply_fill_missing_replace(self):
        ...
        self.assertTrue(data.data['smoking_status'].isna().any())
        
        data.apply_fill_missing("smoking_status", "Replace", "NA")

        miss_df = list(df["smoking_status"].fillna("NA"))
        miss_data = list(data.data['smoking_status'])

        # assert list almost equal
        self.assertTrue(set(miss_df) == set(miss_data))
        
        # all missing values are replaced
        self.assertFalse(data.data['smoking_status'].isna().any())
    ...
    def test_apply_normalize(self):
        ...
        self.assertEqual(data.data['current_ratio'].min(), 0)
        self.assertEqual(data.data['current_ratio'].max(), 1)        

        data.reset_data()
    
        data.apply_normalize("current_ratio", -10, 20)
        self.assertEqual(data.data['current_ratio'].min(), -10)
        self.assertEqual(data.data['current_ratio'].max(), 20)        
        ...
        self.assertEqual(data.filter_data['current_ratio'].min(), 0)
        self.assertEqual(data.filter_data['current_ratio'].max(), 1)        
    ...
    def test_apply_replace(self):
        ...
        self.assertListEqual(list(data.data['sector']), rep_df)
    ...
    def test_apply_replace_str_int(self):
        ...
        data_vals = data['hypertension'].unique().tolist()
    ...
    def test_apply_cell_split_ordered(self):
        ...
        data = DataInMem(filepath, options={'encoding': 'utf-8'})

        data.apply_cell_split('artist_genre', ',', strip="[]", quote="\'\"")
        self.assertTrue('artist_genre_pos_0' in data.columns)
    ...
    def test_apply_cell_split_unordered(self):
        ...
        self.assertTrue('artist_genre_dance pop' in data.columns)
        ...
        self.assertEqual(data['artist_genre_pop'].min(), 0)
        self.assertEqual(data['artist_genre_pop'].max(), 1)

        self.assertEqual(data['artist_genre_zolo'].min(), 0)
        self.assertEqual(data['artist_genre_zolo'].max(), 1)
    ...
    def test_apply_onehot(self):
        ...
        self.assertEqual(data.data['col-0_a'].sum(), N/4)
        self.assertEqual(data.data['col-0_b'].sum(), N/4)
        self.assertEqual(data.data['col-0_c'].sum(), N/4)
        self.assertEqual(data.data['col-0_d'].sum(), N/4)
    ...
    def test_apply_onehot_bind(self):
        ...
        c1a = data.data.loc[data['col-0']=='a', 'col-1'].sum()
        c1b = data.data.loc[data['col-0']=='b', 'col-1'].sum()
        c1c = data.data.loc[data['col-0']=='c', 'col-1'].sum()
        c1d = data.data.loc[data['col-0']=='d', 'col-1'].sum()
        
        self.assertAlmostEqual(data.data['col-0_a_col-1'].sum(), c1a)
        self.assertAlmostEqual(data.data['col-0_b_col-1'].sum(), c1b)
        self.assertAlmostEqual(data.data['col-0_c_col-1'].sum(), c1c)
        self.assertAlmostEqual(data.data['col-0_d_col-1'].sum(), c1d)
    ...
    def test_aggregate_by_index(self):
        ...
        agg_map = [
            ...
            {
                'aggFunc': {'value': "ohe", 'label': "One-hot encoding", 'type': "Nominal"},
                'attrs': [{'value': "col-98", 'label': "col-98", 'type': "Nominal"}],
                'bind': [
                    {
                        'attr': { 'label': "None", 'value': "None", 'type': "Numerical" },
                        'func': { 'value': "mean", 'label': "Mean"}
        ...
        self.assertGreater(result['col-1_mean'].min(), 1)

        # index column should be included
        self.assertTrue('col-0' in result.columns)
        ...
        self.assertTrue('col-98_u_mean' in result.columns)
        self.assertTrue('col-98_v_mean' in result.columns)
        self.assertTrue('col-99_max_count' in result.columns)


        self.assertListEqual(result['col-97_e_col-99_max_count'].tolist(),
                             ['x', 'y', 'y', np.nan])
    ...
    def test_aggregate_by_index_nom_only(self):
        ...
        index_col = 'col-0'
        #agg_type = {c: 'mean' for c in data.columns if c != index_col and c != 'col-2'}
        agg_type = [{
            'aggFunc': {'value': "max_count", 'label': "Most Frequent", 'type': "Nominal"},
            'attrs': [{'value': "col-99", 'label': "col-99", 'type': "Nominal"}]
        }]
    ...
    def test_aggregate_by_index_no_ohe(self):
        """ Test aggregate_by_index method no ohe. """
        filepath = 'test_data/test_data.csv'
        data = DataInMem(filepath)
        data.apply_filter('col-1', lb=1)
        ...
        agg_map = [
            {
                ...
                'attrs': [ {'value': c, 'label': c, 'type': "Numerical"} for c in data.columns \
                        if c not in [index_col, 'col-2'] and c not in data.get_catcols()]
            },
            {
                'aggFunc': {'value': "max_count", 'label': "Most Frequent", 'type': "Nominal"},
                'attrs': [{'value': "col-99", 'label': "col-99", 'type': "Nominal"}]
        ...
        self.assertFalse('col-97_e_col-99_max_count' in data.columns)
        self.assertFalse('col-97_f_col-99_max_count' in data.columns)
        self.assertFalse('col-98_u_mean' in data.columns)
        self.assertFalse('col-98_v_mean' in data.columns)
        self.assertFalse('col-99_max_count' in data.columns)
    ...
    def test_split_data(self):
        ...
        self.assertTrue(isinstance(data, DataInMem))
        
        result3, result4 = data.split_data(sizeType, sizeValue, method)
        self.assertTrue(isinstance(result3, DataInMem))
        self.assertTrue(isinstance(result4, DataInMem))
        
        self.assertEqual(data.shape[1], result1.shape[1])    
        self.assertEqual(data.shape[1], result2.shape[1])
        self.assertEqual(data.shape[0], 374)
        
        # random split gives approximately 112 rows here and the remainder below
        self.assertLessEqual(result1.shape[0], 160) 
    ...
    def test_apply_rank(self):
        ...
        self.assertEqual((data['col-0_rank2_col-24']=='rank_0').sum(), N/2)
        self.assertEqual((data['col-0_rank2_col-24']=='rank_1').sum(), N/2)
    ...
    def test_apply_rank_missing(self):
        ...
        self.assertEqual((data['col-11_rank2_col-24']=='rank_0').sum(), N/2)

        # Last rank excludes missing values
        self.assertEqual((data['col-11_rank2_col-24']=='rank_1').sum(), N/4)
    ...
    def test_apply_rank_bound_missing(self):
        ...
        self.assertTrue(data['col-25'].isna().any())
        
        data.apply_rank('col-0', 'col-25', 2)
        
        self.assertEqual(data.shape[1], M+1)

        self.assertTrue(data['col-0_rank2_col-25'].isna().any())

        self.assertTrue('col-0_rank2_col-25' in data.columns)
        self.assertEqual((data['col-0_rank2_col-25']=='rank_0').sum(), N/2)
        self.assertEqual((data['col-0_rank2_col-25']=='rank_1').sum(), N/4)
    ...
    def test_apply_rank_missing_bound_missing(self):
        ...
        self.assertTrue(data['col-11_rank2_col-25'].isna().any())

        self.assertTrue('col-11_rank2_col-25' in data.columns)
        self.assertEqual((data['col-11_rank2_col-25']=='rank_0').sum(), N/4)
        self.assertEqual((data['col-11_rank2_col-25']=='rank_1').sum(), N/4)
    ...
    def test_value_counts(self):
        ...
        self.assertListEqual(sorted(levels), ['a', 'b', 'c', 'd'])
        ...
        self.assertListEqual(sorted(levels_all), ['a', 'b', 'c', 'd'])
    ...
    def test_value_counts_batch(self):
        ...
        self.assertListEqual(sorted(levels_0), ['a', 'b', 'c', 'd'])
    ...
    def test_detect_types(self):
        ...
        data2 = DataInMem(data=data._data)
        ...
        data_nan = DataInMem(filepath_nan)        
    ...
    def test_bool_type(self):
        ...
        data_bool = DataInMem(filepath_bool)
    ...
    def test_set_type_datetime(self):
        ...
        self.assertTrue((data['col-10'] >= '01-01-1989').all())
    ...
    def test_bayesian_bug(self):
        ...
        self.assertTrue(data['smoking_status'].isna().all())
        ...
        # crashes here
        data.describe()        
        self.assertFalse(data['smoking_status'].isna().any())
