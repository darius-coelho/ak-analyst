...
class AKDataTransformer:
    ...
    def reset_description(self):
        ...
        columns = [
            ...
            "divisionAll", "corr", "coll", "hasHighCard", "card", "ordering"
        ]
    ...
    def __describe_numerical(self, numcols):
        ...
        missCount = self.data.count_na(numcols)
        self.description.loc[numcols, "countMiss"] = missCount
        self.description.loc[numcols, "hasMiss"] = missCount > 0 # set missing status

        nanNumCols = [ c for c in numcols if self.data.shape[0] == missCount[c]]                
        
        self.description.loc[nanNumCols, "min"] = np.nan
        self.description.loc[nanNumCols, "25%"] = np.nan
        self.description.loc[nanNumCols, "50%"] = np.nan
        self.description.loc[nanNumCols, "75%"] = np.nan
        self.description.loc[nanNumCols, "max"] = np.nan
        self.description.loc[nanNumCols, "mean"] = np.nan
        self.description.loc[nanNumCols, "std"] = np.nan
        
        
        logger.info("getting continuous summary statistics")
        numdescr = self.data.describe(list(set(numcols) - set(nanNumCols))).T.drop(['count'], axis=1)        
    ...
    def __describe_nominal(self, catcols):
        ...
        missCount = self.data.count_na(catcols)
        self.description.loc[catcols, "countMiss"] = missCount
        self.description.loc[catcols, "hasMiss"] = missCount > 0 # set missing status)
        ...
        for c, vcounts, vcounts_all in zip(catcols, batch_value_counts, batch_value_counts_all):
            ...
            if not count:  # count list is empty (e.g. includes only nans)
                self.description.at[c, "min"] = 0
                self.description.at[c, "max"] = 0
                self.description.at[c, "mean"] = 0
                self.description.at[c, "std"] = 0
            else:
            ...
            # If ordinal, sort category counts based on ordering
            if self.description.at[c,"type"] == 'Ordinal':                
                ordering = self.data.orderings[c]                 
                self.description.at[c, "ordering"] = ordering                
                
                tmp_count = dict(zip(division, count))                                
                new_counts = [tmp_count[cat] if cat in division else 0 for cat in ordering]
                self.description.at[c, "division"] = ordering
                self.description.at[c, "count"] = new_counts
                self.description.at[c, "divisionPrev"] = ordering
                self.description.at[c, "countPrev"] = new_counts
               
                tmp_count_all = dict(zip(division_all, count_all))
                new_counts_all = [tmp_count_all[cat] if cat in division_all else 0 for cat in ordering]
                self.description.at[c, "divisionAll"] = ordering
                self.description.at[c, "countAll"] = new_counts_all
    ...
    def describe(self):
        ...
        self.description['type'] = [types[c] for c in self.description.index]
        ...
        numcols = self.description[self.description['type'].isin(['Numerical', 'DateTime'])].index.tolist()
    ...
    def __update_column_description_num(self, col):
        ...
        """
        if self.data.shape[0] == self.description.at[col, "countMiss"]:
            self.description.at[col, "min"] = np.nan
            self.description.at[col, "25%"] = np.nan
            self.description.at[col, "50%"] = np.nan
            self.description.at[col, "75%"] = np.nan
            self.description.at[col, "max"] = np.nan
            self.description.at[col, "mean"] = np.nan
            self.description.at[col, "std"] = np.nan
            return
    ...
    @log_it        
    def __update_column_description_datetime(self, col):
        """ Creates the description for the DateTime column type. 
        ...
        for key in desc_dict:
            self.description.at[col, key] = str(desc_dict[key])
        
        count, countAll, division = self.data.histograms_datetime(col, 30)
    ...
    def __update_column_description_cat(self, col):
        ...
        counts_all, division_all = self.data.value_counts_all(col)
        ...
        self.description.at[col, "countAll"] = counts_all
        self.description.at[col, "divisionAll"] = division_all     

        self.description.at[col, "card"] = len(count)    
        self.description.at[col, "hasHighCard"] = len(count) > self.high_card_thresh

        # If ordinal, sort category counts based on ordering
        if self.description.at[col,"type"] == 'Ordinal':
            ordering = self.data.orderings[col] 
            self.description.at[col, "ordering"] = ordering
            
            tmp_count = dict(zip(division, count))                            
            new_counts = [tmp_count[cat] if cat in division else 0 for cat in ordering]
            self.description.at[col, "division"] = ordering
            self.description.at[col, "count"] = new_counts
            self.description.at[col, "divisionPrev"] = ordering
            self.description.at[col, "countPrev"] = new_counts
            
            tmp_count_all = dict(zip(division_all, counts_all))
            new_counts_all = [tmp_count_all[cat] if cat in division_all else 0 for cat in ordering]
            self.description.at[col, "divisionAll"] = ordering
            self.description.at[col, "countAll"] = new_counts_all
    ...
    def update_column_description(self, col):
        ...
        self.description.at[col, 'ordering'] = self.data.orderings[col]

        if self.description.at[col, "type"] == "Numerical":
            self.__update_column_description_num(col)
        elif self.description.at[col, "type"] == "DateTime":
            self.__update_column_description_datetime(col)            
    ...
    def update_description_ohe_transform(self):
        ...
        addedTypes = [types[c] for c in added]

        # add new columns to description index
        self.description = self.description.reindex(columns)
        self.description.loc[added, 'type'] = addedTypes # set appropriate type
        ...
        for c in added:
            self.description.at[c, "count"] = []
        
        # create descriptions for the added columns
        addedNum = [a for a in added if self.description.loc[a, 'type'] == 'Numerical'] 
        addedNom = [a for a in added if self.description.loc[a, 'type'] == 'Nominal']  
        self.__describe_numerical(addedNum)
        self.__describe_nominal(addedNom)
    ...
    def apply_transform(self, transform, **kwargs):
        ...
        if transform_type == 'Log':
        ...
        elif transform_type in ['OHE', 'CellSplit']:
            ...
            self.description.at[trans_attr, 'count'] = np.array([])                        
            self.__describe_numerical(numcols)
            self.__describe_nominal(catcols)
    ...
    def apply_multi_transform(self, mutlitransform, **kwargs):
        ...
        if transform_type == 'Log':
        ...
        elif transform_type == 'OutlierRemoval':
            # Apply transform for each attribute in the list            
            for a in mutlitransform['attr']:
                d = self.description.loc[a]                

                iqr = d["75%"] - d["25%"] 
                lower = d["25%"] - 1.5*iqr
                upper = d["75%"] + 1.5*iqr
                
                lower = lower if lower > d["min"] else d["min"]
                upper = upper if upper < d["max"] else d["max"]
                transform = {
                    'attr': a,
                    'tType': 'Filter',
                    'lb': lower,
            ...
            # Apply transform for each attribute in the list
            if mutlitransform['bind'] in mutlitransform['attr']:
                # Dont allow an attribute to be bound to itself for an OHE transform
                raise ValueError("Bind attribute and OHE attribute cannot be the same.")
            for a in mutlitransform['attr']:
                transform = {
                    'attr': a,
                    'tType': mutlitransform['tType'],
                    'bind': mutlitransform['bind']
                }
                trans_attr = self.transforms.apply_transform(self.data, transform)
            ...
            for a in mutlitransform['attr']:
                transform = {
                    ...
                    'tType': mutlitransform['tType'],
                    'method': mutlitransform['method'],
                    'replaceVal': mutlitransform['replaceVal'],
                }
                trans_attr = self.transforms.apply_transform(self.data, transform)
                # Update descriptions for current attribute
                self.update_column_description(trans_attr)
            ...
            for a in mutlitransform['attr']:
                transform = {
                    'attr': a,
                    'tType': mutlitransform['tType'],
                    'new_type': mutlitransform['new_type']
                }
                if self.description.at[a, 'type'] != transform['new_type']:
                    trans_attr = self.transforms.apply_transform(self.data, transform)
                    self.description.at[trans_attr, 'type'] = transform['new_type']
                    # Update descriptions for current attribute
                    self.update_column_description(trans_attr)
            ...
            for a in mutlitransform['attr']:
                transform = {
                    'attr': a,
                    'tType': mutlitransform['tType'],
                    'expr': mutlitransform['expr'].replace('$var$', a)                   
                }
                trans_attr = self.transforms.apply_transform(self.data, transform)
                # Update descriptions for current attribute
                self.update_column_description(trans_attr)         
    ...
    def update_corr(self):
        ...
        if self.data.shape[1] > 100:
            # don't pre-compute correlation matrix for very high-d data.
            return None 
    ...
    def preview_filter(self, transform):
        ...
        """
        name = transform['attr']
        attr_type = self.description.at[name, 'type']
        
        if attr_type not in ['Numerical', 'DateTime']:
            return  # no preview is necessary

        type_min_lb = {'Numerical': -np.inf, 'DateTime': pd.Timestamp.min}
        type_max_ub = {'Numerical': np.inf, 'DateTime': pd.Timestamp.max}

        lb = transform['lb'] if 'lb' in transform else type_min_lb[attr_type]
        ub = transform['ub'] if 'ub' in transform else type_max_ub[attr_type]

        preview_filter_type = {
            'Numerical': self.data.preview_filter,
            'DateTime': self.data.preview_filter_datetime
        }
        
        count, division, countAll, divisionAll = preview_filter_type[attr_type](name, lb, ub)

        self.description.at[name, "countPrev"] = count
        
        # set the initial full histogram if it does not already exist
        if len(self.description.at[name, 'count']) == 0:
            self.description.at[name, 'count'] = count
            self.description.at[name, 'division'] = division
